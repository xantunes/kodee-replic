...
2. **Tool matcher**: Which tools should trigger it? (`WriteFile`, `Edit`, `Shell`, `*` etc)
...
6. **the agent integration** (CRITICAL): "Should Kimi Code CLI automatically see and fix issues this hook detects?"
...
- Use top-level `additionalContext`/`systemMessage` for the agent communication
...
- **Output**: Using correct top-level JSON structure for the agent communication
...
- Integrates properly with the agent for automated fixes
...
> Run shell commands automatically when Kimi Code CLI edits files, finishes tasks, or needs input. Format code, send notifications, validate commands, and enforce project rules.

Hooks are user-defined shell commands that execute at specific points in Kimi Code CLI's lifecycle. They provide deterministic control over Kimi Code CLI's behavior, ensuring certain actions always happen rather than relying on the LLM to choose to run them. Use hooks to enforce project rules, automate repetitive tasks, and integrate Kimi Code CLI with your existing tools.

For decisions that require judgment rather than deterministic rules, you can also use [prompt-based hooks](#prompt-based-hooks) or [agent-based hooks](#agent-based-hooks) that use a the agent model to evaluate conditions.

For other ways to extend Kimi Code CLI, see [skills](/en/skills) for giving the agent additional instructions and executable commands, [subagents](/en/sub-agents) for running tasks in isolated contexts, and [plugins](/en# [Plugin command - not available in Kimi]s) for packaging extensions to share across projects.
...
The fastest way to create a hook is through the `/hooks` interactive menu in Kimi Code CLI. This walkthrough creates a desktop notification hook, so you get alerted whenever the agent is waiting for your input instead of watching the terminal.

<Steps>
  <Step title="Open the hooks menu">
    Type `/hooks` in the Kimi Code CLI CLI. You'll see a list of all available hook events, plus an option to disable all hooks. Each event corresponds to a point in the agent's lifecycle where you can run custom code. Select `Notification` to create a hook that fires when the agent needs your attention.
  </Step>
  ...
  <Step title="Add your command">
    ...
    <Tabs>
      <Tab title="macOS">
        ...
        ```
        osascript -e 'display notification "Kimi Code CLI needs your attention" with title "Kimi Code CLI"'
        ```
      </Tab>
      ...
      <Tab title="Linux">
        ...
        ```
        notify-send 'Kimi Code CLI' 'Kimi Code CLI needs your attention'
        ```
      </Tab>
      ...
      <Tab title="Windows (PowerShell)">
        ...
        ```
        powershell.exe -Command "[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); [System.Windows.Forms.MessageBox]::Show('Kimi Code CLI needs your attention', 'Kimi Code CLI')"
        ```
      </Tab>
    </Tabs>
  </Step>
  ...
  <Step title="Test the hook">
    Press `Esc` to return to the CLI. Ask the agent to do something that requires permission, then switch away from the terminal. You should receive a desktop notification.
  </Step>
</Steps>

## What you can automate

Hooks let you run code at key points in Kimi Code CLI's lifecycle: format files after edits, block commands before they execute, send notifications when the agent needs input, inject context at session start, and more. For the full list of hook events, see the [Hooks reference](/en/hooks#hook-lifecycle).
...
- [Get notified when the agent needs input](#get-notified-when-claude-needs-input)
...
### Get notified when the agent needs input

Get a desktop notification whenever the agent finishes working and needs your input, so you can switch to other tasks without checking the terminal.

This hook uses the `Notification` event, which fires when the agent is waiting for input or permission. Each tab below uses the platform's native notification command. Add this to `~/.claude/settings.json`, or use the [interactive walkthrough](#set-up-your-first-hook) above to configure it with `/hooks`:
        ...
        "Notification": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "osascript -e 'display notification \"Kimi Code CLI needs your attention\" with title \"Kimi Code CLI\"'"
              }
            ]
          }
        ]
        ...
        "Notification": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "notify-send 'Kimi Code CLI' 'Kimi Code CLI needs your attention'"
              }
            ]
          }
        ]
        ...
        "Notification": [
          {
            ...
            "hooks": [
              {
                "type": "command",
                "command": "powershell.exe -Command \"[System.Reflection.Assembly]::LoadWithPartialName('System.Windows.Forms'); [System.Windows.Forms.MessageBox]::Show('Kimi Code CLI needs your attention', 'Kimi Code CLI')\""
              }
            ]
          }
        ]
...
Automatically run [Prettier](https://prettier.io/) on every file the agent edits, so formatting stays consistent without manual intervention.
...
Prevent the agent from modifying sensitive files like `.env`, `package-lock.json`, or anything in `.git/`. the agent receives feedback explaining why the edit was blocked, so it can adjust its approach.
...
<Steps>
  ...
  <Step title="Make the script executable (macOS/Linux)">
    Hook scripts must be executable for Kimi Code CLI to run them:
  ...
  <Step title="Register the hook">
    Add a `PreToolUse` hook to `.claude/settings.json` that runs the script before any `Edit` or `WriteFile` tool call:
...
When the agent's context window fills up, compaction summarizes the conversation to free space. This can lose important details. Use a `SessionStart` hook with a `compact` matcher to re-inject critical context after every compaction.

Any text your command writes to stdout is added to the agent's context. This example reminds the agent of project conventions and recent work. Add this to `.claude/settings.json` in your project root:
...
Hook events fire at specific lifecycle points in Kimi Code CLI. When an event fires, all matching hooks run in parallel, and identical hook commands are automatically deduplicated. The table below shows each event and when it triggers:

| Event                | When it fires                                        |
| :------------------- | :--------------------------------------------------- |
| `SessionStart`       | When a session begins or resumes                     |
| `UserPromptSubmit`   | When you submit a prompt, before the agent processes it |
| `PreToolUse`         | Before a tool call executes. Can block it            |
| `PermissionRequest`  | When a permission dialog appears                     |
| `PostToolUse`        | After a tool call succeeds                           |
| `PostToolUseFailure` | After a tool call fails                              |
| `Notification`       | When Kimi Code CLI sends a notification                |
...
Each hook has a `type` that determines how it runs. Most hooks use `"type": "command"`, which runs a shell command. Two other options use a the agent model to make decisions: `"type": "prompt"` for single-turn evaluation and `"type": "agent"` for multi-turn verification with tool access. See [Prompt-based hooks](#prompt-based-hooks) and [Agent-based hooks](#agent-based-hooks) for details.

### Read input and return output

Hooks communicate with Kimi Code CLI through stdin, stdout, stderr, and exit codes. When an event fires, Kimi Code CLI passes event-specific data as JSON to your script's stdin. Your script reads that data, does its work, and tells Kimi Code CLI what to do next via the exit code.

#### Hook input

Every event includes common fields like `session_id` and `cwd`, but each event type adds different data. For example, when the agent runs a Bash command, a `PreToolUse` hook receives something like this on stdin:
...
```json  theme={null}
{
  ...
  "tool_name": "Bash",             // the tool the agent is about to use
  "tool_input": {                  // the arguments the agent passed to the tool
...
Your script tells Kimi Code CLI what to do next by writing to stdout or stderr and exiting with a specific code. For example, a `PreToolUse` hook that wants to block a command:
...
if echo "$COMMAND" | grep -q "drop table"; then
  echo "Blocked: dropping tables is not allowed" >&2  # stderr becomes the agent's feedback
...
- **Exit 0**: the action proceeds. For `UserPromptSubmit` and `SessionStart` hooks, anything you write to stdout is added to the agent's context.
- **Exit 2**: the action is blocked. Write a reason to stderr, and the agent receives it as feedback so it can adjust.
- **Any other exit code**: the action proceeds. Stderr is logged but not shown to the agent. Toggle verbose mode with `Ctrl+O` to see these messages in the transcript.
...
<Note>
  Use exit 2 to block with a stderr message, or exit 0 with JSON for structured control. Don't mix them: Kimi Code CLI ignores JSON when you exit 2.
</Note>

For example, a `PreToolUse` hook can deny a tool call and tell the agent why, or escalate it to the user for approval:
...
Kimi Code CLI reads `permissionDecision` and cancels the tool call, then feeds `permissionDecisionReason` back to the agent as feedback. These three options are specific to `PreToolUse`:

- `"allow"`: proceed without showing a permission prompt
- `"deny"`: cancel the tool call and send the reason to the agent
...
For `UserPromptSubmit` hooks, use `additionalContext` instead to inject text into the agent's context. Prompt-based hooks (`type: "prompt"`) handle output differently: see [Prompt-based hooks](#prompt-based-hooks).
...
The `"Edit|Write"` matcher is a regex pattern that matches the tool name. The hook only fires when the agent uses the `Edit` or `WriteFile` tool, not when it uses `Shell`, `ReadFile`, or any other tool.

Each event type matches on a specific field. Matchers support exact strings and regex patterns:

| Event                                                                  | What the matcher filters  | Example matcher values                                                   |
| :--------------------------------------------------------------------- | :------------------------ | :----------------------------------------------------------------------- |
| `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest` | tool name                 | `Shell`, `Edit\|Write`, `mcp__.*`                                         |
| `SessionStart`                                                         | how the session started   | `startup`, `resume`, `clear`, `compact`                                  |
| `SessionEnd`                                                           | why the session ended     | `clear`, `logout`, `prompt_input_exit`, `other`                          |
| `Notification`                                                         | notification type         | `permission_prompt`, `idle_prompt`, `auth_success`, `elicitation_dialog` |
| `SubagentStart`                                                        | agent type                | `Shell`, `Explore`, `Plan`, or custom agent names                         |
...
<Tabs>
  <Tab title="Log every Bash command">
    Match only `Shell` tool calls and log each command to a file. The `PostToolUse` event fires after the command completes, so `tool_input.command` contains what ran. The hook receives the event data as JSON on stdin, and `jq -r '.tool_input.command'` extracts just the command string, which `>>` appends to the log file:
...
| [Plugin](/en# [Plugin command - not available in Kimi]s) `hooks/hooks.json`                   | When plugin is enabled             | Yes, bundled with the plugin       |
| [Skill](/en/skills) or [agent](/en/sub-agents) frontmatter | While the skill or agent is active | Yes, defined in the component file |

You can also use the [`/hooks` menu](/en/hooks#the-hooks-menu) in Kimi Code CLI to add, delete, and view hooks interactively. To disable all hooks at once, use the toggle at the bottom of the `/hooks` menu or set `"disableAllHooks": true` in your settings file.

Hooks added through the `/hooks` menu take effect immediately. If you edit settings files directly while Kimi Code CLI is running, the changes won't take effect until you review them in the `/hooks` menu or restart your session.

## Prompt-based hooks

For decisions that require judgment rather than deterministic rules, use `type: "prompt"` hooks. Instead of running a shell command, Kimi Code CLI sends your prompt and the hook's input data to a the agent model (Haiku by default) to make the decision. You can specify a different model with the `model` field if you need more capability.

The model's only job is to return a yes/no decision as JSON:

- `"ok": true`: the action proceeds
- `"ok": false`: the action is blocked. The model's `"reason"` is fed back to the agent so it can adjust.

This example uses a `Stop` hook to ask the model whether all requested tasks are complete. If the model returns `"ok": false`, the agent keeps working and uses the `reason` as its next instruction:
...
This example verifies that tests pass before allowing the agent to stop:
...
- `Stop` hooks fire whenever the agent finishes responding, not only at task completion. They do not fire on user interrupts.
...
the agent keeps working in an infinite loop instead of stopping.
...
Kimi Code CLI shows a JSON parsing error even though your hook script outputs valid JSON.

When Kimi Code CLI runs a hook, it spawns a shell that sources your profile (`~/.zshrc` or `~/.bashrc`). If your profile contains unconditional `echo` statements, that output gets prepended to your hook's JSON:
...
Kimi Code CLI tries to parse this as JSON and fails. To fix this, wrap echo statements in your shell profile so they only run in interactive shells:
...
> Reference for Kimi Code CLI hook events, configuration schema, JSON input/output formats, exit codes, async hooks, prompt hooks, and MCP tool hooks.

<Tip>
  For a quickstart guide with examples, see [Automate workflows with hooks](/en/hooks-guide).
</Tip>

Hooks are user-defined shell commands or LLM prompts that execute automatically at specific points in Kimi Code CLI's lifecycle. Use this reference to look up event schemas, configuration options, JSON input/output formats, and advanced features like async hooks and MCP tool hooks. If you're setting up hooks for the first time, start with the [guide](/en/hooks-guide) instead.

## Hook lifecycle

Hooks fire at specific points during a Kimi Code CLI session. When an event fires and a matcher matches, Kimi Code CLI passes JSON context about the event to your hook handler. For command hooks, this arrives on stdin. Your handler can then inspect the input, take action, and optionally return a decision. Some events fire once per session, while others fire repeatedly inside the agentic loop:
...
To see how these pieces fit together, consider this `PreToolUse` hook that blocks destructive shell commands. The hook runs `block-rm.sh` before every Shell tool call:
...
<Steps>
  ...
  <Step title="Hook handler runs">
    ...
    If the command had been safe (like `npm test`), the script would hit `exit 0` instead, which tells Kimi Code CLI to allow the tool call with no further action.
  </Step>

  <Step title="Kimi Code CLI acts on the result">
    Kimi Code CLI reads the JSON decision, blocks the tool call, and shows the agent the reason.
  </Step>
</Steps>
...
2. Add a [matcher group](#matcher-patterns) to filter when it fires, like "only for the Shell tool"
...
The matcher is a regex, so `Edit|Write` matches either tool and `Notebook.*` matches any tool starting with Notebook. The matcher runs against a field from the [JSON input](#hook-input-and-output) that Kimi Code CLI sends to your hook on stdin. For tool events, that field is `tool_name`. Each [hook event](#hook-events) section lists the full set of matcher values and the input schema for that event.

This example runs a linting script only when the agent writes or edits a file:
...
- **[Prompt hooks](#prompt-and-agent-hook-fields)** (`type: "prompt"`): send a prompt to a the agent model for single-turn evaluation. The model returns a yes/no decision as JSON. See [Prompt-based hooks](#prompt-based-hooks).
...
All matching hooks run in parallel, and identical handlers are deduplicated automatically. Handlers run in the current directory with Kimi Code CLI's environment. The `$CLAUDE_CODE_REMOTE` environment variable is set to `"true"` in remote web environments and not set in the local CLI.
...
- `${CLAUDE_PLUGIN_ROOT}`: the plugin's root directory, for scripts bundled with a [plugin](/en# [Plugin command - not available in Kimi]s).

<Tabs>
  <Tab title="Project scripts">
    This example uses `$CLAUDE_PROJECT_DIR` to run a style checker from the project's `.claude/hooks/` directory after any `WriteFile` or `Edit` tool call:
  ...
  <Tab title="Plugin scripts">
    ...
    See the [plugin components reference](/en# [Plugin command - not available in Kimi]s-reference#hooks) for details on creating plugin hooks.
  </Tab>
</Tabs>
...
This skill defines a `PreToolUse` hook that runs a security validation script before each `Shell` command:
...
Type `/hooks` in Kimi Code CLI to open the interactive hooks manager, where you can view, add, and delete hooks without editing settings files directly. For a step-by-step walkthrough, see [Set up your first hook](/en/hooks-guide#set-up-your-first-hook) in the guide.
...
Direct edits to hooks in settings files don't take effect immediately. Kimi Code CLI captures a snapshot of hooks at startup and uses it throughout the session. This prevents malicious or accidental hook modifications from taking effect mid-session without your review. If hooks are modified externally, Kimi Code CLI warns you and requires review in the `/hooks` menu before changes apply.
...
The exit code from your hook command tells Kimi Code CLI whether the action should proceed, be blocked, or be ignored.

**Exit 0** means success. Kimi Code CLI parses stdout for [JSON output fields](#json-output). JSON output is only processed on exit 0. For most events, stdout is only shown in verbose mode (`Ctrl+O`). The exceptions are `UserPromptSubmit` and `SessionStart`, where stdout is added as context that the agent can see and act on.

**Exit 2** means a blocking error. Kimi Code CLI ignores stdout and any JSON in it. Instead, stderr text is fed back to the agent as an error message. The effect depends on the event: `PreToolUse` blocks the tool call, `UserPromptSubmit` rejects the prompt, and so on. See [exit code 2 behavior](#exit-code-2-behavior-per-event) for the full list.
...
| `Stop`               | Yes        | Prevents the agent from stopping, continues the conversation |
| `SubagentStop`       | Yes        | Prevents the subagent from stopping                       |
| `PostToolUse`        | No         | Shows stderr to the agent (tool already ran)                 |
| `PostToolUseFailure` | No         | Shows stderr to the agent (tool already failed)              |
...
Exit codes let you allow or block, but JSON output gives you finer-grained control. Instead of exiting with code 2 to block, exit 0 and print a JSON object to stdout. Kimi Code CLI reads specific fields from that JSON to control behavior, including [decision control](#decision-control) for blocking, allowing, or escalating to the user.

<Note>
  You must choose one approach per hook, not both: either use exit codes alone for signaling, or exit 0 and print JSON for structured control. Kimi Code CLI only processes JSON on exit 0. If you exit 2, any JSON is ignored.
</Note>
...
| :--------------- | :------ | :------------------------------------------------------------------------------------------------------------------------- |
| `continue`       | `true`  | If `false`, the agent stops processing entirely after the hook runs. Takes precedence over any event-specific decision fields |
| `stopReason`     | none    | Message shown to the user when `continue` is `false`. Not shown to the agent                                                  |
| `suppressOutput` | `false` | If `true`, hides stdout from verbose mode output                                                                           |
| `systemMessage`  | none    | Warning message shown to the user                                                                                          |

To stop the agent entirely regardless of event type:
...
<Tabs>
  ...
  <Tab title="PreToolUse">
    Uses `hookSpecificOutput` for richer control: allow, deny, or escalate to the user. You can also modify tool input before it runs or inject additional context for the agent. See [PreToolUse decision control](#pretooluse-decision-control) for the full set of options.
...
Each event corresponds to a point in Kimi Code CLI's lifecycle where hooks can run. The sections below are ordered to match the lifecycle: from session setup through the agentic loop to session end. Each section describes when the event fires, what matchers it supports, the JSON input it receives, and how to control behavior through output.

### SessionStart

Runs when Kimi Code CLI starts a new session or resumes an existing session. Useful for loading development context like existing issues or recent changes to your codebase, or setting up environment variables. For static context that does not require a script, use [CLAUDE.md](/en/memory) instead.
...
In addition to the [common input fields](#common-input-fields), SessionStart hooks receive `source`, `model`, and optionally `agent_type`. The `source` field indicates how the session started: `"startup"` for new sessions, `"resume"` for resumed sessions, `"clear"` after `/clear`, or `"compact"` after compaction. The `model` field contains the model identifier. If you start Kimi Code CLI with `claude --agent <name>`, an `agent_type` field contains the agent name.
...
Any text your hook script prints to stdout is added as context for the agent. In addition to the [JSON output fields](#json-output) available to all hooks, you can return these event-specific fields:

| Field               | Description                                                               |
| :------------------ | :------------------------------------------------------------------------ |
| `additionalContext` | String added to the agent's context. Multiple hooks' values are concatenated |
...
Any variables written to this file will be available in all subsequent Bash commands that Kimi Code CLI executes during the session.
...
Runs when the user submits a prompt, before the agent processes it. This allows you
...
| `additionalContext` | String added to the agent's context                                                                                   |
...
Runs after the agent creates tool parameters and before processing the tool call. Matches on tool name: `Shell`, `Edit`, `WriteFile`, `ReadFile`, `Glob`, `Grep`, `Task`, `WebFetch`, `WebSearch`, and any [MCP tool names](#match-mcp-tools).
...
| `permissionDecisionReason` | For `"allow"` and `"ask"`, shown to the user but not the agent. For `"deny"`, shown to the agent                                                       |
...
| `additionalContext`        | String added to the agent's context before the tool executes                                                                                        |
...
| `message`            | For `"deny"` only: tells the agent why the permission was denied                                                  |
| `interrupt`          | For `"deny"` only: if `true`, stops the agent                                                                     |
...
`PostToolUse` hooks can provide feedback to the agent after tool execution. In addition to the [JSON output fields](#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field                  | Description                                                                                |
| :--------------------- | :----------------------------------------------------------------------------------------- |
| `decision`             | `"block"` prompts the agent with the `reason`. Omit to allow the action to proceed            |
| `reason`               | Explanation shown to the agent when `decision` is `"block"`                                   |
| `additionalContext`    | Additional context for the agent to consider                                                  |
...
```json  theme={null}
{
  ...
  "hookSpecificOutput": {
    ...
    "additionalContext": "Additional information for the agent"
  }
}
```
...
Runs when a tool execution fails. This event fires for tool calls that throw errors or return failure results. Use this to log failures, send alerts, or provide corrective feedback to the agent.
...
`PostToolUseFailure` hooks can provide context to the agent after a tool failure. In addition to the [JSON output fields](#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field               | Description                                                   |
| :------------------ | :------------------------------------------------------------ |
| `additionalContext` | Additional context for the agent to consider alongside the error |

```json  theme={null}
{
  "hookSpecificOutput": {
    "hookEventName": "PostToolUseFailure",
    "additionalContext": "Additional information about the failure for the agent"
  }
}
```
...
Runs when Kimi Code CLI sends notifications. Matches on notification type: `permission_prompt`, `idle_prompt`, `auth_success`, `elicitation_dialog`. Omit the matcher to run hooks for all notification types.

Use separate matchers to run different handlers depending on the notification type. This configuration triggers a permission-specific alert script when the agent needs permission approval and a different notification when the agent has been idle:
...
```json  theme={null}
{
  ...
  "message": "the agent needs your permission to use Shell",
...
Runs when a Kimi Code CLI subagent is spawned via the Task tool. Supports matchers to filter by agent type name (built-in agents like `Shell`, `Explore`, `Plan`, or custom agent names from `.claude/agents/`).
...
Runs when a Kimi Code CLI subagent has finished responding. Matches on agent type, same values as SubagentStart.
...
Runs when the main Kimi Code CLI agent has finished responding. Does not run if
the stoppage occurred due to a user interrupt.

#### Stop input

In addition to the [common input fields](#common-input-fields), Stop hooks receive `stop_hook_active`. This field is `true` when Kimi Code CLI is already continuing as a result of a stop hook. Check this value or process the transcript to prevent Kimi Code CLI from running indefinitely.
...
`Stop` and `SubagentStop` hooks can control whether the agent continues. In addition to the [JSON output fields](#json-output) available to all hooks, your hook script can return these event-specific fields:

| Field      | Description                                                                |
| :--------- | :------------------------------------------------------------------------- |
| `decision` | `"block"` prevents the agent from stopping. Omit to allow the agent to stop      |
| `reason`   | Required when `decision` is `"block"`. Tells the agent why it should continue |

```json  theme={null}
{
  "decision": "block",
  "reason": "Must be provided when the agent is blocked from stopping"
}
```

### PreCompact

Runs before Kimi Code CLI is about to run a compact operation.
...
Runs when a Kimi Code CLI session ends. Useful for cleanup tasks, logging session
...
In addition to Bash command hooks (`type: "command"`), Kimi Code CLI supports prompt-based hooks (`type: "prompt"`) that use an LLM to evaluate whether to allow or block an action. Prompt-based hooks work with the following events: `PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`, `UserPromptSubmit`, `Stop`, and `SubagentStop`.

### How prompt-based hooks work

Instead of executing a Bash command, prompt-based hooks:

1. Send the hook input and your prompt to a the agent model, Haiku by default
2. The LLM responds with structured JSON containing a decision
3. Kimi Code CLI processes the decision automatically

### Prompt hook configuration

Set `type` to `"prompt"` and provide a `prompt` string instead of a `command`. Use the `$ARGUMENTS` placeholder to inject the hook's JSON input data into your prompt text. Kimi Code CLI sends the combined prompt and input to a fast the agent model, which returns a JSON decision.

This `Stop` hook asks the LLM to evaluate whether all tasks are complete before allowing the agent to finish:
...
| `reason` | Required when `ok` is `false`. Explanation shown to the agent |

### Example: Multi-criteria Stop hook

This `Stop` hook uses a detailed prompt to check three conditions before allowing the agent to stop. If `"ok"` is `false`, the agent continues working with the provided reason as its next instruction. `SubagentStop` hooks use the same format to evaluate whether a [subagent](/en/sub-agents) should stop:
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "prompt",
            "prompt": "You are evaluating whether the agent should stop working. Context: $ARGUMENTS\n\nAnalyze the conversation and determine if:\n1. All user-requested tasks are complete\n2. Any errors need to be addressed\n3. Follow-up work is needed\n\nRespond with JSON: {\"ok\": true} to allow stopping, or {\"ok\": false, \"reason\": \"your explanation\"} to continue working.",
            "timeout": 30
          }
        ]
      }
...
1. Kimi Code CLI spawns a subagent with your prompt and the hook's JSON input
...
4. Kimi Code CLI processes the decision the same way as a prompt hook
...
This `Stop` hook verifies that all unit tests pass before allowing the agent to finish:
...
By default, hooks block the agent's execution until they complete. For long-running tasks like deployments, test suites, or external API calls, set `"async": true` to run the hook in the background while the agent continues working. Async hooks cannot block or control the agent's behavior: response fields like `decision`, `permissionDecision`, and `continue` have no effect, because the action they would have controlled has already completed.

### Configure an async hook

Add `"async": true` to a command hook's configuration to run it in the background without blocking the agent. This field is only available on `type: "command"` hooks.

This hook runs a test script after every `WriteFile` tool call. the agent continues working immediately while `run-tests.sh` executes for up to 120 seconds. When the script finishes, its output is delivered on the next conversation turn:
...
When an async hook fires, Kimi Code CLI starts the hook process and immediately continues without waiting for it to finish. The hook receives the same JSON input via stdin as a synchronous hook.

After the background process exits, if the hook produced a JSON response with a `systemMessage` or `additionalContext` field, that content is delivered to the agent as context on the next conversation turn.

### Example: run tests after file changes

This hook starts a test suite in the background whenever the agent writes a file, then reports the results back to the agent when the tests finish. Save this script to `.claude/hooks/run-tests-async.sh` in your project and make it executable with `chmod +x`:
...
Then add this configuration to `.claude/settings.json` in your project root. The `async: true` flag lets the agent keep working while tests run:
