...
---
name: create-agent
description: Comprehensive guide for creating Kimi Code CLI agents with proper structure, triggering conditions, system prompts, and validation - combines official Anthropic best practices with proven patterns
...
Create autonomous Kimi Code CLI agents that handle complex, multi-step tasks independently. This command provides comprehensive guidance based on official Anthropic documentation and proven patterns.
...
|---------|-------|---------|
| **Trigger** | the agent decides based on description | User invokes with `/name` |
...
**The most important field** - Defines when the agent triggers the agent.
...
assistant: "[How the agent should respond before triggering]"
<commentary>
[Explanation of why this agent should be triggered in this scenario]
</commentary>
assistant: "[How the agent triggers the agent - 'I'll use the [agent-name] agent...']"
</example>
```
...
3. **Triggers**: When should the agent use this agent?
...
- `# [Plugin command - not available in Kimi]-dev:create-plugin` for full plugin workflow
