...
> **Connections:** No workspace accessible from here. If you're running this from Kimi Code CLI or have a project with files attached, I can fill this in. Want to share where this work lives?
...
|---|---|
| Kimi Code CLI CLI | Glob for files matching dump keywords; Grep for content matches; read top-level structure. Use `scripts/workspace_inventory.py`. |
| the agent.ai with project | Check project knowledge files for thematic overlap. List file titles; surface matches by keyword. |
