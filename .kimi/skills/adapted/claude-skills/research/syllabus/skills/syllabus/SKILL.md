...
> **Portability:** Requires a Consensus MCP connection, Node.js with `docx` package, and file reading capability for the syllabus. Works in Kimi Code CLI CLI natively. In the agent.ai with Consensus MCP + Code Execution + file upload, the workflow is supported.
