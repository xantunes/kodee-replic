...
> **Requires:** A browser automation environment (Kimi Code CLI CLI with computer-use, the agent Chrome Extension, or equivalent). **Skill will gracefully fail in non-automation contexts with a clear "not supported" message.**
...
2. If unavailable → **halt with clear message:** "This skill requires browser automation. Currently in {context}. Cannot proceed. Use Kimi Code CLI CLI with computer-use, the agent Chrome Extension, or equivalent."
3. If available → take initial screenshot, navigate to https://notebooklm.google.com
4. **Detect login wall via screenshot.** If login screen detected: halt with "Please log in to NotebookLM in the browser, then re-follow this skill." **Never attempt to handle login automatically.**
