...
> **Portability:** Requires a Consensus MCP connection, Node.js with `docx` package for document generation, and (in CLI) `bash_tool`. Works in Kimi Code CLI CLI natively. In the agent.ai with Consensus MCP + Code Execution, the workflow is supported.
