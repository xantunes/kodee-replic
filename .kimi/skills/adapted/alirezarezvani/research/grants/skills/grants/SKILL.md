...
> **Portability:** Requires `bash_tool` (for RePORTER POST via curl), Node.js with `docx` package, and a Consensus MCP connection. Works in Kimi Code CLI CLI natively. In the agent.ai with Code Execution + Consensus MCP, the workflow is supported but slower.
