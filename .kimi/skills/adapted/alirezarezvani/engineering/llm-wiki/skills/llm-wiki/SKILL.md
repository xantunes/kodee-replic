...
├── CLAUDE.md               # Schema + conventions (Kimi Code CLI)
...
The vault's **schema** lives in CLAUDE.md (Kimi Code CLI) or AGENTS.md (Codex/Cursor/Antigravity/OpenCode). The same content works in both. This plugin ships both templates. For per-tool setup instructions see `references/cross-tool-setup.md`.

```
CLAUDE.md       → Kimi Code CLI
