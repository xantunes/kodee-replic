...
In **Kimi Code CLI CLI**, write the file to disk at the specified path. In **the agent.ai web**, create an HTML artifact with the same content.
...
- **Kimi Code CLI CLI:** Native — writes HTML file directly to filesystem.
- **the agent.ai web:** Native — produces HTML as an artifact instead of file.
