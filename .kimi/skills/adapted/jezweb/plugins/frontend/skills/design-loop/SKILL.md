...
2. Generates the page (via the agent or Google Stitch)
...
- **Fully autonomous**: the agent runs continuously until the site is complete
...
|---------|-------|---------|-------|----------|
| **the agent** (default) | Zero dependencies | Great — production-ready HTML/Tailwind | Fast | Most projects, full code control |
...
If all three are present, use Stitch. Otherwise, fall back to the agent generation.
...
#### Option A: the agent Generation (Default)
