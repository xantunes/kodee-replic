...
---
name: mcp-builder
description: "Build MCP servers in Python with FastMCP. Define tools / resources / prompts, build the server, test locally, deploy to FastMCP Cloud or Docker. Use whenever the user mentions building an MCP server, exposing tools to LLMs, FastMCP, building a the agent integration, or troubleshooting FastMCP module-level server, storage, lifespan, middleware, OAuth, or deployment errors."
...
- **Tools** -- Functions the agent can call (API wrappers, calculations, file operations)
- **Resources** -- Data the agent can read (database records, config, documents)
...
For Kimi Code CLI terminal use, add scripts alongside the MCP server:

```
my-mcp-server/
├── src/index.ts          # MCP server (for the agent.ai)
├── scripts/
...
fastmcp install server.py          # Install to the agent Desktop
