...
2. **Scope**: Whatever the agent has been working on in the current session. Look at recent context: files edited, decisions made, architecture discussed, problems being solved.
...
| "challenge this" / "devil's advocate" | Devil's advocate (1 model) | the agent's current position |
...
**Avoid using for**:
- Simple syntax checks (the agent handles these)
...
5. **Use AI-to-AI framing** — the model is advising the agent, not talking to the human
