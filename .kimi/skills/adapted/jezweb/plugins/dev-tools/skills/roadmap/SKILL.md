...
Generate a comprehensive technical roadmap for building an entire application. Detailed enough that Kimi Code CLI can pick up any phase and execute it autonomously for hours.
...
- Planning a multi-week build that will span many Kimi Code CLI sessions
- Before saying "build this" — the roadmap is what you hand Kimi Code CLI to execute
...
- **Be completable in 1-3 Kimi Code CLI sessions** — if a phase takes more than a day, split it
...
### Task Checklist
[Actionable tasks grouped by area — these are what Kimi Code CLI executes]
...
The file should be a single markdown document that Kimi Code CLI can read at the start of any session and know exactly what to build next. It's the project's north star.
...
6. **Tasks are grouped by layer** (data, API, frontend, infra) — Kimi Code CLI works in layers, not features.
7. **Each phase has a definition of done** — specific things to test and verify.
8. **Include the stack table** — don't make the agent guess the tech choices per phase.
