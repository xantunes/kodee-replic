...
| **React (Vite)** | @vitejs# [Plugin command - not available in Kimi]-react, react-dom | `jsdom` or `happy-dom` |
...
/// <reference types="vitest/config" />
import { defineConfig } from "vite";
import react from "@vitejs# [Plugin command - not available in Kimi]-react";
