...
compatibility: Designed for Kimi Code CLI (or similar products)
---

# Codex Review

Run an independent code review via the OpenAI Codex CLI (`codex review`). The value is a second opinion from a **different model family** than the one that wrote the code — Codex catches things the agent misses due to author bias.
...
- **Don't leak the agent's reasoning into the prompt.** The `prompt.md` file is deliberately neutral — Codex reviews the code, not the agent's narrative about the code. Independence is the whole point.
...
- the agent surfaces the top findings without auto-fixing them
